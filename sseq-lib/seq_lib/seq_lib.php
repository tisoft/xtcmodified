...
function SEQ_SECURE_SESSION() {
    global $_SEQ_SESSIONLIFETIME, $_SEQ_SESSIONABSOLUTELIFETIME,
           ...
           $_SEQ_SECURE_COOKIES, $HTTP_SESSION_VARS;
...
function SEQ_TERMINATE_SESSION_($redir_exit = true) {
    global $_SEQ_ONERROR_REDIRECT_TO, $_SEQ_SECURE_COOKIES, $HTTP_COOKIE_VARS;
    
    $seq_sessname = session_name();
    
    // expire cookie
    if ($_SEQ_SECURE_COOKIES && ($_COOKIE || $HTTP_COOKIE_VARS)
        && isset($_COOKIE[$seq_sessname]) && !headers_sent())
    {
        setcookie($seq_sessname, '', time()-42000, '/');
...
function SEQ_CREATE_TOKEN_NAME_($originname_ = '') {
    ...
    $originname = $originname_ ? md5($originname_ . $header_hash . _SEQ_APP_SALT) : md5($header_hash . _SEQ_APP_SALT);
...
function SEQ_FTOKEN($formname_ = '', $once_ = false) {
    return '<input type="hidden" name="' . SEQ_CREATE_TOKEN_NAME_($formname_) . '" value="' . SEQ_CREATE_TOKEN_VALUE_($formname_, $once_) . '" />' . "\n";
}
...
function seq_useragent_fingerprint_() {
    ...
    seq_debug_($_SERVER['HTTP_USER_AGENT'].$_SERVER['HTTP_ACCEPT_LANGUAGE'].
               _SEQ_APP_SALT);
...
function SEQ_ISNUM($string_ = '', $minvalue_ = null, $maxvalue_ = null, $varname_ = '', $source_ = '') {
    ...
    if ($typ_numeric) {
        for ($t=0; $t < count($minvallist); $t++) {
            ...
            if (isset($minvalue) && strtoupper($minvalue) != 'NULL' && $string_ < $minvalue) {
                seq_log_(($varname_ ? $varname_ : 'UNKNOWN VAR') . ': INT below MIN (' . $minvalue . ')', $string_, $source_);
                seq_reaction_();
                return false;
            }
        }
        if (isset($maxvalue_) && $string_ > $maxvalue_) {
...
function SEQ_ISSTR($string_ = '', $minvalue_ = null, $maxvalue_ = null, $varname_ = '', $source_ = '') {
    ...
    if ($typ_string) {
        if (isset($minvalue_) && strlen($string_) < $minvalue_) {
            seq_log_(($varname_ ? $varname_ : 'UNKNOWN VAR') . ': STR length below MIN (' . $minvalue_ . ')', $string_, $source_);
            seq_reaction_();
            return false;
        }
        if (isset($maxvalue_) && strlen($string_) > $maxvalue_) {
 ...
 */
function seq_reaction_() {
    global $_SEQ_IDS_ONATTACK_ACTION;
    
    $action_array = split(' ', $_SEQ_IDS_ONATTACK_ACTION);
...
function SEQ_DATADUMP() {
    global $_SEQ_BASEDIR, $HTTP_GET_VARS, $HTTP_POST_VARS, $HTTP_COOKIE_VARS;
        ...
        foreach($sanitizeList_ as $varname=>$actions) {
            ...
            if (preg_match('/g/i', $source)) {
                if (isset($_GET) && isset($_GET[$varname])) {
                    $_GET[$varname] = sanitize_var_($_GET[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, '_GET', $sql, $xss);
                }              
                if (ini_get('register_long_arrays') && isset($HTTP_GET_VARS) && isset($HTTP_GET_VARS[$varname])) {
                    $HTTP_GET_VARS[$varname] = sanitize_var_($HTTP_GET_VARS[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, 'HGET', $sql, $xss);
                }
            }

            if (preg_match('/p/i', $source)) {                  
                if (isset($_POST) && isset($_POST[$varname])) {
                    $_POST[$varname] = sanitize_var_($_POST[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, '_POS', $sql, $xss);                  
                }
                if (ini_get('register_long_arrays') && isset($HTTP_POST_VARS) && isset($HTTP_POST_VARS[$varname])) {
                    $HTTP_POST_VARS[$varname] = sanitize_var_($HTTP_POST_VARS[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, 'HPOS', $sql, $xss);
                }
            }

            if (preg_match('/c/i', $source)) {
                if (isset($_COOKIE) && isset($_COOKIE[$varname])) {
                    $_COOKIE[$varname] = sanitize_var_($_COOKIE[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, '_COO', $sql, $xss);                  
                }                  
                if (ini_get('register_long_arrays') && isset($HTTP_COOKIE_VARS) && isset($HTTP_COOKIE_VARS[$varname])) {
                    $HTTP_COOKIE_VARS[$varname] = sanitize_var_($HTTP_COOKIE_VARS[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, 'HCOO', $sql, $xss);                    
                }
            }

            if (preg_match('/s/i', $source) && isset($_SESSION)) {
                if (isset($_SESSION) && isset($_SESSION[$varname])) {
                    $_SESSION[$varname] = sanitize_var_($_SESSION[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, '_SES', $sql, $xss);
                }
                if (ini_get('register_long_arrays') && isset($HTTP_SESSION_VARS) && isset($HTTP_SESSION_VARS[$varname])) {
                    $HTTP_SESSION_VARS[$varname] = sanitize_var_($HTTP_SESSION_VARS[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, 'HSES', $sql, $xss);
                }
            }
            
            if (isset($_REQUEST) && isset($_REQUEST[$varname])) {
                $_REQUEST[$varname] = sanitize_var_($_REQUEST[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, '_REQ', $sql, $xss);
            }
            
            if (isset($GLOBALS) && isset($GLOBALS[$varname])) {
                $GLOBALS[$varname] = sanitize_var_($GLOBALS[$varname], strtoupper(trim($actions['type'])), $actions['min'], $actions['max'], $varname, ' GLO', $sql, $xss);
            }
