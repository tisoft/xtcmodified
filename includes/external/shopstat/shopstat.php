...
if ((!(isset($shoplog_mode) && $shoplog_mode)) && ((isset($shoplog_method) && $shoplog_method == 'buy1') || (isset($shoplog_method) && $shoplog_method == 'buy2') || (isset($shoplog_method) &&
  $shoplog_method == 'buy3') || (isset($shoplog_method) && $shoplog_method == 'listitems') || (isset($shoplog_method) && $shoplog_method == 'showitem') || (isset($shoplog_method) &&
  $shoplog_method == 'search'))) {
  if ($shopstat_debug) {
    $shopstat_debug_msg .= '\nMETHOD:\n------------\n'.$shoplog_method;
    $shopstat_debug_msg .= '\nRESULT:\n------------\nNOTLOGGED!';
...
//BOF - DokuMan - 2011-01-21 - Fix a notice when there is no sessionId found in request
//$shoplog_id = $_REQUEST[session_name()];
$sessionName = session_name();
$shoplog_id = '';
if(isset($_REQUEST[$sessionName])) {
  $shoplog_id = $_REQUEST[$sessionName];
}
//EOF - DokuMan - 2011-01-21 - Fix a notice when there is no sessionId found in request
...
elseif (isset($shoplog_method) && $shoplog_method == 'validate_and_send') {
  ...
  if (is_object($order)) {
    ...
    $shoplog_query = ((isset($order->info['total']))?$order->info['total'] : '').'&amp;'.((isset($order->info['payment_method']))?$order->info['payment_method'] : '').'&amp;'.((isset($order->
      info['shipping_cost']))?$order->info['shipping_cost'] : '').'->';
    ...
    if (isset($order->products)) {
      ...
      foreach ($order->products as $item) {
        ...
        $shoplog_query .= implode('&amp;', array($item['model'], urlencode(strip_tags($item['name'])), urlencode(strip_tags(shopstat_getRealPath(xtc_get_product_path(xtc_get_prid($item['id']))))),
          $item['qty'], $item['price']));
        $init = 0;
      }
    }
...
if ($logit && !(isset($shoplog_islogged) && $shoplog_islogged)) {
      ...
      if (file_exists($sl_logfilename_last)) { # pack theold file ...
        ...
        if ($sl_fpout = gzopen($sl_dest, 'wb9')) {
          if ($sl_fpin = fopen($sl_logfilename_last, 'rb')) {
            while (!feof($sl_fpin))
              gzwrite($sl_fpout, fread($sl_fpin, 1024 * 512));
            fclose($sl_fpin);
          } else {
            $sl_error = true;
          }
  ...
  $sl_log = implode("\t", array($sl_datestr, $sl_timestr, $shoplog_ip, str_replace(" ", "+", $shoplog_useragent), $shoplog_referer, urlencode($shoplog_id), $shoplog_method, urlencode
    ($shoplog_cat), $shoplog_query))."\n";
