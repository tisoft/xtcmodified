...
/* -----------------------------------------------------------------------------------------
   ...
   (c) 2009 - 2009 ECB Shop-Software, J�rg Wedekind <yogi@ecb-shop.de>
   (c) 2010 xtc-Modified
...
// Auff�llen der Konto-Nr. mit '0' auf 10 Stellen
  ...
  function AccountInfo($AccountNo) {
  ...
  function Method00($AccountNo, $Significance, $Checkpoint, $Modulator=10, $expandAccount=true) {
    $Help = 0;
    $Method00 = 1;
...
/* ----- Die Funktionen der einzelnen Berechnungsmethoden ---- */
  ...
  function Mark24($AccountNo) {
    ...
    switch (substr($AccountNo,0,1)) {
      ...
      case 5 :
      case 6 :
        // deaktivert, da die Postbank diese Definition nicht einh�lt
  ...
  function Mark66($AccountNo) {
    $Significance = '070065432'; //@author J�rg Wedekind <yogi@ecb-shop.de>
    $Help = 0;
    $Mark66 = 1;
    $AccountNo = $this->ExpandAccount($AccountNo);
    for ($Run = 0;$Run < strlen($Significance);$Run++) {
...
//Result 2 und 4 (Bankdaten nicht pr�fbar) als OK zur�ckgeben
