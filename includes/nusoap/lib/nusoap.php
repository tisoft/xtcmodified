...
class nusoap_base {
	...
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
		...
		switch(true) {
			...
			case (is_array($val) || $type):
                ...
                if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
					$i = 0;
	...
	*/
	function expandQname($qname){
		// get element prefix
		if(strpos($qname,':') && !preg_match('/^[http://]/',$qname)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
			// get unqualified name
...
function timestamp_to_iso8601($timestamp,$utc=true){
	...
	if($utc){
		...
		if(preg_match('/'.$eregStr.'/',$datestr,$regs)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
...
class XMLSchema extends nusoap_base  {
	...
	function schemaStartElement($parser, $name, $attrs) {
        ...
        if(count($attrs) > 0){
        	foreach($attrs as $k => $v){
				...
				if(preg_match("/^xmlns/",$k)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
                	//$this->xdebug("$k: $v");
		...
		switch($name){
			...
			case 'complexType':
				...
				if(isset($attrs['name'])){
					...
					//                </sequence>
					//            </complexType>
					if(isset($attrs['base']) && preg_match('/:Array$/',$attrs['base'])){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
    ...
    */
	function getTypeDef($type){
		...
		} elseif (preg_match('/_ContainedType$/', $type)) { // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function soap_transport_http($url){
		parent::nusoap_base();
		$this->setURL($url);
		preg_match('/\$Revisio' . 'n: ([^ ]+)/', $this->revision, $rev); // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
	...
	function getResponse(){
	  ...
	  if ($this->scheme == 'http' || $this->scheme == 'ssl') {
	    ...
	    while (!isset($lb)){
			...
			// remove 100 header
			if(isset($lb) && preg_match('/^[HTTP/1.1 100]/',$data)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
				unset($lb);
				$data = '';
			}//
		}
...
//		}
		...
		// remove 100 header(s)
		while (preg_match('/^[HTTP/1.1 100]/',$data)) { // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
			if ($pos = strpos($data,"\r\n\r\n")) {
	...
	function parseCookie($cookie_str) {
		...
		$data = explode(';', $cookie_str); // Hetfield - 2009-08-18 - replaced depricated function split with explode to be ready for PHP >= 5.3
		$value_str = $data[0];
...
class soap_server extends nusoap_base {
	...
	*/
	function service($data){
		...
		if (preg_match('/wsdl/', $qs) ){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
	...
	function parse_http_headers() {
		...
		if(function_exists('getallheaders')){
			...
			if(isset($this->headers['content-type']) && strpos($this->headers['content-type'],'=')){
				...
				if(preg_match('/^(ISO-8859-1|US-ASCII|UTF-8)$/i',$enc)){ // Hetfield - 2009-08-19 - replaced depricated function eregi with preg_match to be ready for PHP >= 5.3
...
class wsdl extends nusoap_base {
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
        ...
        } elseif (preg_match('/schema$/', $name)) { // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
            ...
            if (preg_match('/:/', $name)) { // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
                // get ns prefix
	...
	*/
	function end_element($parser, $name){ 
		// unset schema status
		if (/*preg_match('/types$/', $name) ||*/ preg_match('/schema$/', $name)) { // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
			$this->status = "";
            $this->appendDebug($this->currentSchema->getDebug());
            $this->currentSchema->clearDebug();
	...
	*/
	function start_element($parser, $name, $attrs) {
		...
		foreach($attrs as $key => $value){
			...
			// if ns declarations, add to class level array of valid namespaces
            if($key_prefix == 'xmlns'){
				if(preg_match('^[http://www.w3.org/][0-9]{4}[/XMLSchema]$/',$value)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
			...
			// if it's a type declaration, set type
				...
				*/
				...
				if(preg_match('/'.$expr.'/',$value,$regs)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
...
class soapclientw extends nusoap_base  {
	...
	*/
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		...
		switch(true){
			// http(s)
			case preg_match('/^http/',$this->endpoint): // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
				...
				if(preg_match('/^http:/',$this->endpoint)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
				//if(strpos($this->endpoint,'http:')){
					$this->responseData = $http->send($msg,$timeout,$response_timeout,$this->cookies);
				} elseif(preg_match('/^https/',$this->endpoint)){ // Hetfield - 2009-08-19 - replaced depricated function ereg with preg_match to be ready for PHP >= 5.3
