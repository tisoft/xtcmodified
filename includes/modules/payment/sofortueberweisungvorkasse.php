...
// class constructor
    function sofortueberweisungvorkasse() {
      global $order;
      ...
      $this->sort_order = MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_STATUS == 'True') ? true : false);
      $this->redirect = false;
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_ZONE > 0) ) {
        $check_flag = false;
        $check_query = xtc_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function checkout_success($insert_id) {
      ...
      $parameter= array();
      $return = '';
		  $parameter['kdnr']	= MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_KDNR;  // Repräsentiert Ihre Kundennummer bei der Sofortüberweisung
		  $parameter['projekt'] = MODULE_PAYMENT_SOFORTUEBERWEISUNGVORKASSE_PROJEKT;  // Die verantwortliche Projektnummer bei der Sofortüberweisung, zu der die Zahlung gehört
