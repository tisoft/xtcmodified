<?php
  ...
  ***************************************************************/
          ...
          while ($rows = xtc_db_fetch_array($rows_query)) {
            ...
            foreach ($table_list as $column) {
              ...
              if (!isset($rows[$column])) {
                ...
                $insert.='\''.xtc_db_input($rows[$column]).'\','; // DokuMan - 2010-01-28 - instead of mysql_escape_string
              } else {
                $insert.='\'\',';
              }
          ...
          while ($rows = xtc_db_fetch_array($rows_query)) {
            ...
            foreach ($table_list as $column) {
              ...
              if (!isset($rows[$column])) {
                ...
                $values[] ='\''.xtc_db_input($rows[$column]).'\''; // DokuMan - 2010-01-28 - instead of mysql_escape_string
              } else {
                $values[] ='\'\'';
              }
              //BOF NEW TABLE  STRUCTURE  - LIKE MYSQLDUMPER
  ...
  if ($_GET['action'] == 'backupnow') {
    ...
    //xtc_db_query('set names utf8'); ONLY FOR UTF8
