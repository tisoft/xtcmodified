    ...
    if($xtb_module=='cats') {
      ...
      if(isset($_SESSION['xtb1']['multi_settings'])) {
        ...
        while($x1 = xtc_db_fetch_array($products_query)) 
          ...
          // Zeichensatz pruefen
          if(!mb_check_encoding($item['TITLE'],'UTF-8')) { $item['TITLE'] = utf8_encode($item['TITLE']); }
          if(!mb_check_encoding($item['SUBTITLE'],'UTF-8')) { $item['SUBTITLE'] = utf8_encode($item['SUBTITLE']); }
          if(!mb_check_encoding($item['DESCRIPTION'],'UTF-8')) { $item['DESCRIPTION'] = utf8_encode($item['DESCRIPTION']); }
                                  ...
                                  <form method="post" action="xtbooster.php" name="sd" id="xsb_add_form" onsubmit="xsb.post(this);return false;">
                                    ...
                                    <table border="0" cellpadding="2" cellspacing="0" width="100%" id="TABLE_MAIN">
                                      ...
                                      if(!$multi_xtb) {
                                        ...
                                        <?php
                                        if(strlen(strip_tags($x['products_name'])) > 54) {
                                          ?>
                                          <tr class="attributes<?php echo $rowi++%2==0?'-even':'-odd'; ?>">
